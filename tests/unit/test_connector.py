...
def test_connector_get_without_context_manager(console_fixture):
    connector = Connector(console_fixture)
    with pytest.raises(RuntimeError):
        connector.get(url="", params={})


def test_connector_get_return_json(mock_requests_fixture, console_fixture):
...
@patch("main.sleep")
def test_connector_get_raise_for_status(mock_sleep, mock_requests_fixture, console_fixture):
    ...
    assert mock_requests_fixture.call_count == 3
